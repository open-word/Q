...
declare @SumPAYB decimal(18,4)			= (select sum(Budget) from PAYB);
declare @SumPADYB decimal(18,4)			= (select sum(Budget) from PADYB);
...
select @YBudgetCheck = count(1) from 
	...
	(select YearID, sum(Budget) [YBudget] from PAYB group by YearID) c on a.YearID = c.YearID and a.YBudget = c.YBudget left join
	(select YearID, sum(Budget) [YBudget] from PADYB group by YearID) d on a.YearID = d.YearID and a.YBudget = d.YBudget left join
...
select @PYBudgetCheck = count(1) from
	...
	(select ProgrammeID, YearID, sum(Budget) PYBudget from PAYB group by ProgrammeID, YearID) b on a.ProgrammeID = b.ProgrammeID and a.YearID = b.YearID and a.PYBudget = b.PYBudget left join
	(select ProgrammeID, YearID, sum(Budget) PYBudget from PADYB group by ProgrammeID, YearID) c on a.ProgrammeID = c.ProgrammeID and a.YearID = c.YearID and a.PYBudget = c.PYBudget left join
...
select @PAYBudgetCheck = count(1) from
	(select ProgrammeID, ActorID, YearID, sum(Budget) PAYBudget from PAYB group by ProgrammeID, ActorID, YearID) a left join
	(select ProgrammeID, ActorID, YearID, sum(Budget) PAYBudget from PADYB group by ProgrammeID, ActorID, YearID) b on a.ProgrammeID = b.ProgrammeID and a.ActorID = b.ActorID and a.YearID = b.YearID and a.PAYBudget = b.PAYBudget left join
